...
- **Orquestrador**: `baixar_semana.py` baixa todos os mercados da semana em sequência (idempotente). Com `--extrair` roda também o pipeline completo: `ler_produtos_gemini.py --todas` → `migrar_categorias.py` → `normalizar_produtos.py`. `--sem-normalizar` pula as duas últimas. `facebook` (login manual) e `pinterest` ficam fora por padrão (`--todos` inclui).
